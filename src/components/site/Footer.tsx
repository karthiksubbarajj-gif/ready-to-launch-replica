    ...
    <footer className="mt-24 border-t border-border/50 bg-surface/50">
      <div className="container-page py-14 grid gap-10 md:grid-cols-2 lg:grid-cols-5">
        <div className="lg:col-span-1">
          ...
          <div className="mt-5 flex gap-3">
            {[Instagram, Facebook, Linkedin, Youtube].map((Icon, i) => (
              <a key={i} href="#" className="h-9 w-9 grid place-items-center rounded-full bg-surface-2 border border-border text-orange hover:bg-orange hover:text-background transition" aria-label="social">
        ...
        <FooterCol title="Quick Links" links={[
          ...
          { to: "/projects", label: "Projects" }, { to: "/courses", label: "Courses" },
        ...
        <FooterCol title="Our Services" links={[
          { to: "/services", label: "Digital Marketing" }, { to: "/services", label: "Staffing Solutions" },
          { to: "/services", label: "Campaign Management" }, { to: "/services", label: "Website Development" },
...
function FooterCol({ title, links, extra }: { title: string; links: { to: string; label: string }[]; extra?: { to: string; label: string }[] }) {
  return (
    <div>
